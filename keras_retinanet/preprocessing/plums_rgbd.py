...
from ..utils.image import read_image_bgr, read_depth_image
...
class PascalRGBDGenerator(Generator):
    """ Generate data for a Pascal VOC format RGBD dataset.
    ...
    def __init__(
        ...
        **kwargs
        ...
        """ Initialize an RGBD data generator.
        ...
        super(PascalRGBDGenerator, self).__init__(**kwargs)
    ...
    def depth_path(self, image_index):
        """ Get the path to a depth image.
        """
        return os.path.join(self.data_dir, 'DepthImages', self.image_names[image_index] + self.depth_extension)
    ...
    def load_depth(self, image_index):
        """ Load a depth image at the image_index.
        """
        return read_depth_image(self.depth_path(image_index))

    def load_rgbd(self, image_index):
        """ Load an RGBD image at the image_index.
        """
        rgb = self.load_image(image_index)
        # print("rgb size {}".format(rgb.shape))
        d = self.load_depth(image_index)
        d = np.expand_dims(d, axis=2)
        # print("d size {}".format(d.shape))
